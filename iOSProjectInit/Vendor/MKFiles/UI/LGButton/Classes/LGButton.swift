...
open class LGButton: UIControl {
    ...
    @IBInspectable public var btnBorderColor: UIColor = UIColor.white {
        didSet{
            setupView()
        }
    }
    ...
    @IBInspectable public var btnShadowOffset: CGSize = CGSize.init(width:0, height:0) {
        didSet{
            setupView()
        }
    }
    
    @IBInspectable public var btnShadowRadius: CGFloat = 0 {
        didSet{
            setupView()
        }
    }
    
    @IBInspectable public var btnShadowOpacity: CGFloat = 1 {
        didSet{
            setupView()
        }
    }
    
    @IBInspectable public var btnShadowColor: UIColor = UIColor.black {
        didSet{
            setupView()
        }
    }
    ...
    fileprivate func setupBackgroundColor() {
        bgContentWidthConstraint.constant = min(0, -btnBorderWidth)
        bgContentHeightConstraint.constant = min(0, -btnBorderWidth)
    ...
    fileprivate func setupBorderAndCorners() {
        if fullyRoundedCorners {
            ...
            bgContentView.layer.cornerRadius = btnCornerRadius
    ...
    fileprivate func setupTitle() {
        ...
        if !titleString.isEmpty {
            ...
            if let font = font {
                titleLbl.font = font
            } else if let titleFont = titleFontName {
                titleLbl.font = UIFont.init(name:titleFont , size:titleFontSize)
            }else{
                titleLbl.font = UIFont.systemFont(ofSize: titleFontSize)
            }
        } else if let attributedString = attributedString, !attributedString.string.isEmpty {
    ...
    fileprivate func setupShadow(){
        if btnShadowRadius > 0 {
            layer.shadowOffset = btnShadowOffset
            layer.shadowRadius = btnShadowRadius
            layer.shadowOpacity = Float(btnShadowOpacity)
            layer.shadowColor = btnShadowColor.cgColor
        }
    }
